...
class BuildGFGRUStack(nn.Module):
    ...
    def __init__(self, input_size, rnn_size, num_layers):
        ...
        l_uij_lst = [[nn.Linear(self.rnn_size, self.rnn_size)] * self.num_layers] * self.num_layers
        self.l_wg = [nn.ModuleList([nn.Linear(self.input_size, 1)] * self.num_layers)]
        self.l_ug = [nn.ModuleList(l_ug_lst[0])]
        self.l_uij = [nn.ModuleList(l_uij_lst[0])]
    ...
    def forward(self, x, prev_hs):
        ...
        self.h_stacked = prev_hs[0]
        for L in range(1,self.num_layers):
            self.h_stacked = torch.cat((self.h_stacked, prev_hs[L]),1)
